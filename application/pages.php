...
class Pages_Controller extends Controller
{
	public function __construct($app)
	{
		...
		$this->app->get_page()->set_path(array(
			...
			'index.php?route='.MODULE_NAME => 'Pages',
			));	
		
		$columns = array(
			array('db_name' => 'id',          'column_name' => 'Id',          'sorting' => 1),
			array('db_name' => 'main_page',   'column_name' => 'Main Page',      'sorting' => 1),
			array('db_name' => 'system_page', 'column_name' => 'System Page',   'sorting' => 1),
			array('db_name' => 'caption',     'column_name' => 'Category',   'sorting' => 1),
			array('db_name' => 'title',       'column_name' => 'Title',       'sorting' => 1),
			array('db_name' => 'contents',    'column_name' => 'Contents',       'sorting' => 1),
			array('db_name' => 'description', 'column_name' => 'Description',        'sorting' => 1),
			array('db_name' => 'previews',    'column_name' => 'Views',      'sorting' => 1),
		...
		if ($this->app->get_acl()->allowed(OPERATOR)) // są uprawnienia
		{
			...
			if (!empty($_POST))
			{
				...
				if (isset($_POST['save_button']))
				{
					...
					if ($id) $this->app->get_page()->set_message(MSG_INFORMATION, 'Post has been succesfully created');
					else $this->app->get_page()->set_message(MSG_ERROR, 'Post was not created');
		...
		if ($this->app->get_acl()->allowed(OPERATOR)) // są uprawnienia
		{
			...
			if (!empty($_POST))
			{
				...
				if (isset($_POST['save_button']))
				{
					...
					if ($result) $this->app->get_page()->set_message(MSG_INFORMATION, 'Changes has been succesfully updated.');
	...
	public function Delete_Action()
	{
		if ($this->app->get_acl()->allowed(OPERATOR)) // są uprawnienia
		{
			...
			if (isset($_GET['confirm']))
			{
				...
				if ($result) $this->app->get_page()->set_message(MSG_INFORMATION, 'Post has been succesfully deleted.');
				else $this->app->get_page()->set_message(MSG_ERROR, 'Post not deleted.');
	...
	public function Archive_Action()
	{
		if ($this->app->get_acl()->allowed(OPERATOR)) // są uprawnienia
		{
			...
			if ($result) $this->app->get_page()->set_message(MSG_INFORMATION, 'Post has been succesfully archived.');
			else $this->app->get_page()->set_message(MSG_ERROR, 'Post not archived.');
		...
		if ($this->app->get_acl()->allowed(OPERATOR)) // są uprawnienia
		{
			...
			if (!empty($_POST))
			{
				...
				if (isset($_POST['restore_button']))
				{
					...
					if ($result) $this->app->get_page()->set_message(MSG_INFORMATION, 'Post has been succesfully restored.');
					else $this->app->get_page()->set_message(MSG_ERROR, 'Post not restored.');
