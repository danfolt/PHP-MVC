...
class Sites_Controller extends Controller
{
	public function __construct($app)
	{
		...
		$this->app->get_page()->set_path(array(
			...
			'index.php?route='.MODULE_NAME => 'Pages - Snippets',
			));	
		
		$columns = array(
			array('db_name' => 'id',          'column_name' => 'Id',          'sorting' => 1),
			array('db_name' => 'main_page',   'column_name' => 'Home Page',      'sorting' => 1),
			array('db_name' => 'system_page', 'column_name' => 'System page',   'sorting' => 1),
			array('db_name' => 'category_id', 'column_name' => 'Category',   'sorting' => 1),
		...
		if ($this->app->get_acl()->allowed(OPERATOR)) // są uprawnienia
		{
			...
			if (!empty($_POST))
			{
				...
				if (isset($_POST['save_button']))
				{
					...
					if ($id) $this->app->get_page()->set_message(MSG_INFORMATION, 'Page was succesfully created');
					else $this->app->get_page()->set_message(MSG_ERROR, 'Page was not created');
