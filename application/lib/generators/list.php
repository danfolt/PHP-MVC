...
class ListBuilder extends Builder
{
	...
	public function build_list()
	{
		...
		{
			$this->columns[] = array(
				'db_name' => NULL, 'column_name' => 'Actions', 'sorting' => NULL,
				);
		}
		...
		$main_text .= '</div>';
		$main_text .= '<button type="submit" name="ListSearchButton" id="ListSearchButton" class="btn btn-default">Search</button>';
		$main_text .= '</form>';
		$main_text .= '</span>';
		if ($this->dates)
		{
		...
		if (!empty($_SESSION['list_filter']))
		{
			...
			$main_text .= '<span class="FormSearchCaption">Search:</span>&nbsp;';
