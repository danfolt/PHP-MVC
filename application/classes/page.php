...
class Page
{
	...
	public function get_logged()
	{
		...
		if ($this->app->get_user()->get_value('user_status'))
		{
			...
			$line .= 'Welcome, '. $status['user_name'] .' '. $status['user_surname'] .' - (<a href="index.php?route=logout">Log Out</a>)';
		}
		else
		{
			$line .= 'You are not logged in';
		}
	...
	public function get_navbar()
	{
		...
		if (count($this->navbar))
		{
			foreach ($this->navbar as $k => $v)
			{
				...
				if ($access) // Tab Unavailable - Public
				{
					$result .= '<li class="'.$class_name.'-public">'.'<a href="'.$link.'" '.$option.'>'.$caption.'</a>'.'</li>';
				}
				else // Tab Unavailable - Private
				{
					$result .= '<li class="'.$class_name.'-private">'.'<a href="'.$link.'" '.$option.'>'.'<i>'.$caption.'</i>'.'</a>'.'</li>';
				}
			}
		}
