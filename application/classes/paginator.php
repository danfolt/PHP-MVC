...
class List_Paginator
{
	...
	public function show()
	{
		...
		$output .= '<button class="btn btn-default" name="navi" type="submit" value="go">Submit</button>';
		$output .= '</span>';
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</form>';
		...
		$output .= 'Position: <b>' . number_format($_SESSION['result_capacity'], 0, ',', '.') . '</b>';
		$output .= '&nbsp; ▪ &nbsp;';
		$output .= 'Pages: <b>' . number_format($_SESSION['page_counter'], 0, ',', '.') . '</b>';
		$output .= '</td>';
